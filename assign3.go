...
type Request struct {
	Initial_loc  string   `json:"StartPt"`
...
type Res1 struct {
	...
	Initial_loc      string   `json:"starting_from_location_id" bson:"starting_from_location_id"`
...
func C_Loc(rw http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	route := append(res.Bestlocation_ids, req.Initial_loc)
	...
	//fmt.Println("Matrix TSP : ", Adj_mat)
	...
	for i := 1; i <= len(wayPoints)-2; i++ {
		...
		j := wayPoints[i]
		//	fmt.Println("Location to be appended", route[j])
		res.Bestlocation_ids = append(res.Bestlocation_ids, route[j])
		//	fmt.Println("Optimized route : ", j, res.Bestlocation_ids)
	}
	...
	res.Status = "Planning"
	res.Initial_loc = req.Initial_loc
	res.Costs = Costs
...
func put_Location(rw http.ResponseWriter, r *http.Request, p httprouter.Params) {
	...
	//fmt.Println(Index_Put, len(req.Bestlocation_ids))
	...
	if Index_Put < len(req.Bestlocation_ids) {
		...
		mmm_res2.Initial_loc = req.Initial_loc
...
func main() {
	...
	http.ListenAndServe("localhost:6000", hi)
